...
import com.skyretro.iptv.databinding.ActivitySeriesDetailBinding
...
class SeriesDetailActivity : AppCompatActivity() {
        ...
        seasons.forEach { season ->
            ...
            tab.setOnFocusChangeListener { _, hasFocus ->
                ...
                tab.setBackgroundColor(when {
                    hasFocus && !isSelected -> 0xFF2D6090.toInt()
                    isSelected -> 0xFFFFCC00.toInt()
                    else -> 0xFF1A3A6A.toInt()
                })
                tab.setTextColor(when {
                    isSelected -> 0xFF000080.toInt()
    ...
    private fun selectSeason(season: String) {
        ...
        for (i in 0 until container.childCount) {
            ...
            tab.setBackgroundColor(if (isSelected) 0xFFFFCC00.toInt() else 0xFF1A3A6A.toInt())
            tab.setTextColor(if (isSelected) 0xFF000080.toInt() else 0xFFFFFFFF.toInt())
        }
    ...
    private fun onEpisodeLongPress(ep: Episode) {
        ...
        val options = if (isFav)
            arrayOf("REMOVE FROM FAVOURITES", "CANCEL")
        else
            arrayOf("▶ PLAY", "★ ADD TO FAVOURITES", "CANCEL")
    ...
    private fun onPlayEpisode(ep: Episode) {
        val ext   = ep.containerExtension ?: "mp4"
        val url   = repository.buildSeriesEpisodeUrl(serverUrl, username, password, ep.id, ext)

...
object FavouritesManager {
    ...
    fun getLiveChannels(ctx: Context): List<FavouriteItem> =
        getAll(ctx).filter { it.type == com.orbital.iptv.data.model.FavType.LIVE }

    fun containsLive(ctx: Context, streamId: Int): Boolean =
        getAll(ctx).any { it.type == com.orbital.iptv.data.model.FavType.LIVE && it.streamId == streamId }

    fun addLiveChannel(ctx: Context, name: String, streamId: Int, streamUrl: String, iconUrl: String?) {
        addOrUpdate(ctx, com.orbital.iptv.data.model.FavouriteItem(
            id = "live_$streamId",
            type = com.orbital.iptv.data.model.FavType.LIVE,
            title = name, artUrl = iconUrl ?: "", streamUrl = streamUrl, streamId = streamId
        ))
    }

    fun removeLive(ctx: Context, streamId: Int) {
        save(ctx, getAll(ctx).filter { !(it.type == com.orbital.iptv.data.model.FavType.LIVE && it.streamId == streamId) })
    }
